...
本 skill 支持三种方式配置 API Key，优先级从高到低依次为：

1. **命令行参数** `--api-key`
2. **环境变量** `IWENCAI_API_KEY`
3. **本地配置文件** `config.json`（推荐，一次配置即可长期使用）

#### 方式一：配置文件（推荐）

skill 根目录下已提供 `config.json` 模板，将你的 API Key 填入即可：
...
每次调用时直接传入：
```bash
python3 scripts/cli.py --query "银行股" --api-key "your-api-key"
```
...
> **首次使用 - 获取 API Key**
>
> 步骤 1：在浏览器内打开同花顺 i 问财 SkillHub 页面：https://www.iwencai.com/skillhub
>
> 步骤 2：登录同花顺账号
>
> 步骤 3：点击具体的 Skill，打开弹窗查看详情，在安装方式「Agent 用户」处找到您的 `IWENCAI_API_KEY` 并复制
>
> 步骤 4：将复制的 API Key 填入 skill 根目录的 `config.json` 中，或配置为环境变量
...
| Header | 取值说明 |
|--------|----------|
| `Authorization` | `Bearer <API Key>`，API Key 从 `--api-key` 参数、环境变量 `IWENCAI_API_KEY` 或 skill 根目录 `config.json` 中读取 |
...
# API Key 获取优先级：环境变量 > 配置文件 > 手动指定
api_key = os.environ.get("IWENCAI_API_KEY") or "your-api-key"
...
| `--api-key` | STRING | 否 | API 密钥（默认从环境变量或 skill 根目录 `config.json` 读取）|
...
- **密钥缺失（config.json、环境变量、--api-key 均未配置）**：
  代理必须**口头提示**用户「使用前」中的完整 API Key 获取指引文案，即：
  > 首次使用 - 获取 API Key
  > 本技能支持三种方式配置 API Key（优先级由高到低）：
  > 1. 命令行参数 `--api-key`
  > 2. 环境变量 `IWENCAI_API_KEY`
  > 3. skill 根目录 `config.json` 中的 `api_key` 字段（推荐）
  >
  > 获取步骤：
  > 步骤 1：在浏览器内打开同花顺 i 问财 SkillHub 页面：https://www.iwencai.com/skillhub
  >
  ...
  > 步骤 3：点击具体的 Skill，打开弹窗查看详情，在安装方式「Agent 用户」处找到您的 `IWENCAI_API_KEY` 并复制
  >
  > 步骤 4：将 API Key 填入 skill 根目录的 `config.json` 中，或配置为环境变量
...
├── SKILL.md              # Skill 使用指南
├── LICENSE.txt           # 许可证文件
├── config.json           # 本地配置文件（存放 IWENCAI_API_KEY）
└── scripts/

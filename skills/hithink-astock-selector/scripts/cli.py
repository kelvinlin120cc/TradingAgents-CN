...
- Authorization Bearer 从 --api-key 参数、环境变量 IWENCAI_API_KEY 或 skill 根目录 config.json 读取
- 优先使用 POST
...
# config.json 位于 skill 根目录（scripts 的父目录）
...
def _read_config_file(config_path: str) -> str:
    """从本地 config.json 读取 api_key，返回空字符串表示未找到或为空。"""
    try:
...
def get_api_key(cli_api_key: Optional[str]) -> str:
    """获取 API 密钥：优先 CLI 参数，其次环境变量，最后读取 skill 目录下的 config.json。"""
    key = cli_api_key or os.environ.get("IWENCAI_API_KEY", "")
    if not key:
        key = _read_config_file(_CONFIG_PATH)
    if not key:
        raise AStockAPIError(
            "API 密钥未设置。请通过以下任一方式配置：\n"
            "1) 在 skill 目录的 config.json 中填入 api_key；\n"
            "2) 通过 --api-key 参数传入；\n"
            "3) 设置环境变量 IWENCAI_API_KEY。\n"

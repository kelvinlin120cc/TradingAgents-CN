...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        system_prompt = (
            ...
            "📊 【报告格式要求】（强制执行）："
            "   - 必须保留工具返回数据中的【数据概览】表格，包含数据类别、数据时间、数据来源、更新时间"
            "   - 所有指标表格必须包含4列：指标、数值、数据时间、说明（不要用'解读'列）"
            "   - 必须在报告中明确标注每个财务指标的数据时间（如：PE_TTM的数据时间、ROE的报告期）"
            "   - 必须在报告底部明确标注数据来源（MongoDB/Tushare/AKShare）和数据更新时间"
            "   - 如果工具数据中包含时间和来源信息，必须原样保留，不得删除或修改"
            "可用工具：{tool_names}。\n{system_message}"
            "当前日期：{current_date}。"
            ...
            if current_tool_calls > 0:
                ...
                if has_tool_result:
                    ...
                    force_system_prompt = (
                        ...
                        f"报告必须包含以下内容：\n"
                        f"1. 公司基本信息和财务数据分析\n"
                        f"2. PE_TTM、PB、PEG等估值指标分析\n"
                        f"3. 当前股价是否被低估或高估的判断\n"
                        f"4. 合理价位区间和目标价位建议\n"
                        f"5. 基于基本面的投资建议（买入/持有/卖出）\n\n"
                        f"📊 【报告格式要求】（强制执行）：\n"
                        f"   - 必须保留工具返回数据中的【数据概览】表格，包含数据类别、数据时间、数据来源、更新时间\n"
                        f"   - 所有指标表格必须包含4列：指标、数值、数据时间、说明（不要用'解读'列）\n"
                        f"   - 必须在报告中明确标注每个财务指标的数据时间（如：PE_TTM的数据时间、ROE的报告期）\n"
                        f"   - 必须在报告底部明确标注数据来源（MongoDB/Tushare/AKShare）和数据更新时间\n"
                        f"   - 如果工具数据中包含时间和来源信息，必须原样保留，不得删除或修改\n\n"
                        f"要求：\n"
                        f"- 使用中文撰写报告\n"
                        f"- 基于消息历史中的真实数据进行分析\n"
                        f"- 分析要详细且专业\n"
